...
function App() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (response.status === 'success' && response.data) {
      ...
      setStep('form');
      setToast({ 
        message: response.exists ? 'Records Auto-Fetched from Database' : 'Identity Verified from Master List', 
        type: 'success' 
      });
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasError) {
        setErrors(newErrors);
        setToast({ message: 'Validation Failed: Please review required fields', type: 'error' });
        return;
    }
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex flex-col items-center justify-start py-8 px-4 bg-slate-50">
          ...
          <div className="max-w-md mx-auto animate-slideUp" style={{ animationDelay: '0.1s' }}>
            <div className="bg-white rounded-[3rem] shadow-[0_40px_100px_rgba(0,0,0,0.1)] border border-slate-100 p-12 relative overflow-hidden">
              ...
              <form onSubmit={handleLogin} className="space-y-8">
                ...
                >
                  {loading ? <Loader2 className="w-7 h-7 animate-spin" /> : <><LogIn className="w-6 h-6" /> SECURE LOGIN</>}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="animate-slideUp">
            <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-4">
              ...
              <div className="flex items-center gap-4 bg-slate-950 text-white px-8 py-4 rounded-2xl font-black shadow-2xl">
                ...
                <span className="tracking-widest uppercase text-sm">ENCRYPTED ID: {data.hrmsId}</span>
              </div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
              {/* VIRTUAL ID CARD - AUTO-REFLECTS FETCHED DATA */}
                      ...
                      <div className="space-y-8">
                        <div>
                          ...
                          <div className="mt-4 flex flex-col gap-1">
                            <p className="text-white/30 text-[9px] font-black uppercase tracking-widest">HINDI NAME (AUTO-FETCHED)</p>
              ...
              {/* DATA MANAGEMENT INTERFACE - AUTO-POPS FORM FIELDS */}
                ...
                <form onSubmit={handleSubmit} className="bg-white rounded-[4rem] p-10 md:p-16 shadow-[0_20px_60px_rgba(0,0,0,0.05)] border border-slate-100">
                  <div className="flex items-center gap-6 mb-16">
                    ...
                    <div>
                      ...
                      <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-1">Live Database Synchronization</p>
                    </div>
                  </div>

                  {/* AUTO-FETCHED MASTER SUMMARY */}
                  <div className="mb-16">
                    <div className="flex items-center gap-2 text-slate-950 font-black text-xs uppercase tracking-widest mb-6">
                       <CheckCircle2 className="w-4 h-4 text-emerald-600" /> MASTER LIST DATA (VERIFIED)
                    </div>
                  ...
                  <div className="mb-16">
                    <div className="flex items-center gap-2 text-slate-950 font-black text-xs uppercase tracking-widest mb-6">
                       <Upload className="w-4 h-4 text-indigo-600" /> IDENTITY ASSET {data.photo && <span className="text-[10px] ml-2 px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full font-bold">Cloud Synced</span>}
                    </div>
                    <div className="flex flex-col md:flex-row items-center gap-10 bg-indigo-50/10 p-10 rounded-[3rem] border-2 border-dashed border-indigo-200/50">
                      ...
                      <div className="flex-1 w-full space-y-6">
                        <p className="text-sm text-slate-500 font-bold leading-relaxed">
                          Your profile photo is auto-fetched if it exists in the 'Data' sheet. New uploads will replace the existing file on save.
                        </p>
                        <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full flex items-center justify-center gap-4 px-8 py-6 bg-slate-950 text-white rounded-[1.8rem] font-black text-lg hover:bg-indigo-700 shadow-2xl shadow-indigo-100 transition-all active:scale-95">
                          <Upload className="w-6 h-6" /> {data.photo ? 'CHANGE EXISTING PHOTO' : 'UPLOAD NEW PHOTO'}
                        </button>
                  ...
                  {/* EDITABLE FIELDS - AUTO-FETCHED VALUES POPULATE HERE */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
                    ...
                    <div className="md:col-span-2">
                      <div className="bg-slate-50 p-8 rounded-[2.5rem] border-2 border-slate-100 mb-8 focus-within:border-indigo-600 focus-within:bg-white transition-all group relative">
                        ...
                        />
                        {data.hindiName && <span className="absolute top-8 right-12 text-[9px] font-black text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full uppercase tracking-widest border border-emerald-100">Synced</span>}
                      </div>
                    </div>
                    ...
                    <div className="relative group">
                      ...
                      {data.adharNumber && !errors.adharNumber && isExisting && <span className="absolute top-1.5 right-12 text-[8px] font-black text-emerald-600 opacity-60 uppercase tracking-widest pointer-events-none">Cloud Synced</span>}
                    </div>
                    ...
                    <div className="relative group">
                      ...
                      {data.panNumber && !errors.panNumber && isExisting && <span className="absolute top-1.5 right-12 text-[8px] font-black text-emerald-600 opacity-60 uppercase tracking-widest pointer-events-none">Cloud Synced</span>}
                    </div>
                    ...
                    <div className="md:col-span-2 relative group">
                       ...
                       {data.epicNumber && !errors.epicNumber && isExisting && <span className="absolute top-1.5 right-14 text-[8px] font-black text-emerald-600 opacity-60 uppercase tracking-widest pointer-events-none">Cloud Synced</span>}
                    </div>
                    ...
                    <div className="relative group">
                      ...
                      {data.mobileNumber && !errors.mobileNumber && isExisting && <span className="absolute top-1.5 right-12 text-[8px] font-black text-emerald-600 opacity-60 uppercase tracking-widest pointer-events-none">Cloud Synced</span>}
                    </div>
                    ...
                    <div className="relative group">
                      ...
                      {data.gmailId && !errors.gmailId && isExisting && <span className="absolute top-1.5 right-12 text-[8px] font-black text-emerald-600 opacity-60 uppercase tracking-widest pointer-events-none">Cloud Synced</span>}
                    </div>
                  </div>
                  ...
                  <div className="mt-20 flex items-center justify-end">
                    ...
                    >
                      {loading ? (
                        ...
                        <>
                          {isExisting ? (
                            <><RefreshCw className="w-7 h-7 group-hover:rotate-180 transition-transform duration-700" /> UPDATE EXISTING RECORD</>
                          ) : (
                            <><Save className="w-7 h-7 group-hover:scale-125 transition-transform" /> INITIALIZE PROFILE</>
                          )}
      ...
      <footer className="mt-20 mb-12 text-slate-400 font-black text-[10px] uppercase tracking-[0.4em] text-center leading-loose border-t border-slate-200 pt-10 w-full max-w-4xl">
        HRMS EXECUTIVE SYNC PORTAL • BUILD V6.6.0<br/>
        <span className="opacity-50 text-[8px]">STRICT SINGLE-ROW OVERWRITE PROTOCOL ACTIVE</span>
      </footer>
    </div>
  );
}
